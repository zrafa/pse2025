...
void thread_pse(controllerClass & control) {
  ...
  while (1) 
  {
    int value = joypad.sensor();
    float gl_x = -2.5f*(((value*2)/255.f) - 1);
		//std::cout << "Hola mundo, value: " << value << " - " << gl_x << std::endl;
		//std::this_thread::sleep_for(std::chrono::microseconds(100));  // 2 seg
      control.movePaddle(gl_x);
  }
}
