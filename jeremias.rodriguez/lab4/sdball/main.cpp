...
float mapValue(int value, int izq = 1023, int der = 0, float out_min = -1.66f, float out_max = 1.66f) {
  if (value > izq) value = izq;
  if (value < der) value = der;

  return (value - izq) * (out_max - out_min) / (float)(der - izq) + out_min;
}
...
void thread_pse(controllerClass & control) {
	float p = -1;
  int last_value = 0, num = 0;
  std::string device_path = "/dev/ttyUSB0"; // Ruta por defecto

  std::ifstream serial_device(device_path);
  if (!serial_device.is_open()) {
      std::cerr << "Error al abrir el dispositivo " << device_path << std::endl;
  }

  std::cout << "Leyendo datos desde " << device_path << "..." << std::endl;

  std::string line;
  while (1) {
    if (std::getline(serial_device, line)) {
        ...
        try {
            if (line[0] == ' ') {
                ...
                if (std::abs(last_value - num) > 3) {
                    std::cout << "Un número: " << num << "\n" << std::endl;
                    p = mapValue(num);  // Mapear a rango -1.66 a 1.66
                    control.movePaddle(p);
                    last_value = num;
                }
            } else {
                std::cerr << "Línea no válida: " << line << std::endl;
            }
        } catch (const std::exception& e) {
            std::cerr << "Error al convertir línea: \"" << line << "\" -> " << e.what() << std::endl;
        }
    }
}
